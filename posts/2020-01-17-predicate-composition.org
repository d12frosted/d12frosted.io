...
implement the following functions and investigate how far we can go into
abstractions until performance degrades. Or maybe it wont degrade. Who knows?
...
it. We use =-O= to enable optimisations (you can also use =-O2=) and
=-ddump-simpl= to dump the simplified output, which can be combined with
=-ddump-to-file= to write result into a file instead of =stdout=. More options
are described in the [[https://downloads.haskell.org/ghc/latest/docs/html/users_guide/debugging.html#debugging-the-compiler][GHC manual]].
...
This prints a lot of stuff, so let me focus on the most important part. I am
showing you only several lines out of 267 total. You can view the full file
[[https://github.com/d12frosted/d12frosted.io/blob/master/assets/snippets/predicate-composition/inline.dump-simpl][here]].
...
Interestingly, if you pass =-v= option to =ghc=, it will also print
...
If we compile it, the relevant part in Core language is unchanged (full output
can be found [[https://github.com/d12frosted/d12frosted.io/blob/master/assets/snippets/predicate-composition/operator-naive.dump-simpl][here]]).
...
While our code looks better, there are no runtime penalties. However in some
cases you should explicitly use [[https://downloads.haskell.org/ghc/latest/docs/html/users_guide/glasgow_exts.html#inline-pragma][INLINE pragma]]. In short, with =-O= option GHC
always tries to inline small functions (based on [[https://downloads.haskell.org/ghc/latest/docs/html/users_guide/using-optimisation.html#ghc-flag--funfolding-creation-threshold=%E2%9F%A8n%E2%9F%A9][unfolding-creation-threshold]]
and heuristics) thus avoiding the call overhead and enabling other optimisations
(like replacing all expression with it's result). And when unfolding doesn't
happen for some of the reasons and you really think that it should happen (make
such decision based on CPU and memory profiling), then put inline pragma.
...
Thanks to =Semigroup= the =plus= function is not passed implicitly and =gmagic=
...
So it turns out that our =gmagic= function is binary operator from =Semigroup=.
How convenient, isn't it?

And we know that \((\mathcal{B}, \wedge)\) forms semigroup. As well as
\((\mathcal{B}, \vee)\). Since in Haskell you can't explicitly pass type class
instance, people usually use =newtype= wrappers when a given type has different
lawful instances. Like with booleans and different operators. So [[https://hackage.haskell.org/package/base-4.12.0.0/docs/Data-Monoid.html#g:2][Data.Monoid]]
exports such wrappers for us -- =All= (with =&&=) and =Any= (with =||=).
...
[[https://github.com/d12frosted/d12frosted.io/blob/master/assets/snippets/predicate-composition/operator-newtype.dump-simpl][inspecting the dumped]] Core we can confirm that this version is not different
...
But what about =newtype= like =All= and =Any=? We know that they are isomorphic
to =Bool= (and mutually as well). And here comes the second kind of type
equality -- representational. They all share the same representation. While
=All= and =Bool= are representationally equal, they are not equal nominally!
...
-- 'I will return you a value of type which can be converted to =Bool='. We can
fix the type by using the function in obvious context[fn:3]:
...
So let's look at [[https://github.com/d12frosted/d12frosted.io/blob/master/assets/snippets/predicate-composition/operator-coerce.dump-simpl][the Core dump]].
...
But we can refer to criterion to make it more visible.
...
#+ATTR_HTML: :height 40 :xAxisBeginAtZero true
#+begin_criterion
#+end_criterion
...
[fn:2] Kinds?
[fn:3] Or we can use [[https://gitlab.haskell.org/ghc/ghc/wikis/type-application][TypeApplications]] extension.
