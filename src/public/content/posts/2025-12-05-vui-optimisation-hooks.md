...
| Hook           | Purpose                                      |
|----------------|----------------------------------------------|
| `use-ref`      | Mutable value that doesn't trigger re-render |
| `use-callback` | Stable function reference across renders     |
| `use-memo`     | Cached computation result                    |
...
That's what `use-ref` provides.

## How It Works

`use-ref` returns a cons cell. The `car` holds your value:
...
When you create a timer in `use-effect` with dependencies, you need `use-ref` to share the timer handle between the effect body and its cleanup:

``` elisp
(defcomponent polling-fetcher (url)  ; url is a dependency!
  :state ((data nil))
  :render
  (let ((timer-ref (use-ref nil)))
    (use-effect (url)  ; Re-runs when url changes
...
The cleanup was defined when the previous timer was created, so it captured that `let` binding. With `use-ref`, both the old cleanup and the new effect share the same mutable cell.

**When you don't need use-ref:** If your timer setup runs exactly once, `:on-mount` with a local `let` is simpler:

``` elisp
(defcomponent simple-timer ()
  :state ((seconds 0))
  :on-mount
...
``` elisp
(defcomponent filtered-count-tracker (items filter)
  :render
  (let ((prev-count-ref (use-ref nil))
...
With `:on-update` you'd have to reconstruct the previous computed value from `prev-props`. With `use-ref`, you simply remember whatever you want.

The trade-off: `:on-update` is automatic and declarative; `use-ref` is manual but works for any value.

### Counting Renders (Debugging)

``` elisp
(defcomponent render-counter ()
  :state ((count 0))
  :render
  (let ((renders (use-ref 0)))
...
``` elisp
(defcomponent ref-demo ()
  :state ((trigger 0))
  :render
  (let ((clicks (use-ref 0)))
    (vui-vstack
...
> **What you learned:** `use-ref` gives you a mutable container that persists across renders without triggering re-renders. Use it for timer handles, previous values, and any data that shouldn't affect the UI directly.
...
``` elisp
(defcomponent child-button (label on-click)
  :should-update
...
`use-callback` returns a stable function reference that only changes when its dependencies change:
...
(defcomponent render-tracking-button (label on-click)
  :should-update
...
(defcomponent callback-demo ()
  :state ((other 0))
  :render
  (let ((stable-click (use-callback ()
...
> **What you learned:** `use-callback` returns a stable function reference that only changes when dependencies change. This prevents unnecessary re-renders in optimised child components.

# Cached Computations - `use-memo`

`use-memo` caches the result of an expensive computation, only recomputing when dependencies change.
...
(defcomponent filtered-list ()
  :state ((items (generate-large-list))
          (filter-text ""))
  :render
  (let ((filtered (use-memo (items filter-text)
...
Without `use-memo`, the filter would run on *every* render - even if neither `items` nor `filter-text` changed.
...
## The Difference from `use-callback`

| Hook           | Returns    | Caches                 |
|----------------|------------|------------------------|
| `use-callback` | A function | The function reference |
| `use-memo`     | Any value  | The computation result |

You *could* implement `use-callback` with `use-memo`:

``` elisp
;; These are equivalent:
(use-callback (x) (lambda () (process x)))
(use-memo (x) (lambda () (process x)))
```

But `use-callback` makes the intent clearer.

## Try It

``` elisp
(defcomponent memo-demo ()
  :state ((numbers (number-sequence 1 1000))
          (threshold 500)
          (unrelated 0))
  :render
  (let ((filtered (use-memo (numbers threshold)
...
> **What you learned:** `use-memo` caches computation results, only recomputing when dependencies change. Use it for expensive operations like filtering large lists.

# Custom Comparison Modes

Both `use-callback` and `use-memo` have starred variants that accept a `:compare` option:
...
``` elisp
(defcomponent mode-display (mode)  ; mode is a symbol like 'edit or 'view
  :render
  (let ((mode-config (use-memo* (mode)
                       :compare 'eq
...
``` elisp
(defcomponent length-sensitive (items)
  :render
  (let ((processed (use-memo* (items)
...
``` elisp
(defcomponent compare-demo ()
  :state ((symbol 'alpha)
          (counter 0))
  :render
  (let ((with-eq (use-memo* (symbol)
                   :compare 'eq
                   (message "eq memo computed")
                   (format "Symbol: %s" symbol)))
        (with-equal (use-memo* (symbol)
                      :compare 'equal
...
(when show-details
  (use-memo (data) (process data)))  ; Don't do this!

;; GOOD: always call, conditionally use result
(let ((processed (use-memo (data) (process data))))
  (when show-details
    (vui-text processed)))
```
...
4.  Uses `use-memo` to avoid re-filtering on unrelated state changes
5.  Tracks total keystrokes with `use-ref` (for analytics, without re-rendering)
6.  Shows both the filtered count and keystroke count

``` elisp
(defcomponent search-panel ()
  :state ((query "")
