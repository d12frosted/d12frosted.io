...
The time has come to release a new version of [Vulpea](https://github.com/d12frosted/vulpea), a collection of note-taking functions based on Org and Org Roam. Almost a year has passed since the release of v0.2. Over this time, Vulpea has received several major features that deserve a dedicated version. In truth, I wanted to release it almost half a year ago, but the war kicked in, and I had more important things to focus on.

The main focus of this release is the [performance of query operations](https://github.com/d12frosted/vulpea#query-from-database). Expect a 4.5x speed boost for generic `vulpea-db-query` and even more with specialised queries. All that thanks to a materialised view table for notes.
...
Check out [CHANGELOG](https://github.com/d12frosted/vulpea/blob/master/CHANGELOG.org) for a full list of changes. Read further to learn the details.

<!--more-->

# Materialized view table

Materialised view table, huh? The concept is actually pretty simple. But first, you need to understand how Org Roam DB is designed.
...
This penalty can be omitted if the [materialised view table becomes a part of Org Roam](https://github.com/org-roam/org-roam/issues/1997). Alternatively, the Org Roam routine can benefit from refactoring to enable this kind of hook without any performance loss. Do I plan to work in this direction? Definitely! But I feel a lack of motivation and resources to promise anything.

See [vulpea#116](https://github.com/d12frosted/vulpea/pull/116) for benchmarks.

# Specialised queries

As I already mentioned, all this performance mumbo jumbo started because of [the inclusion of links](https://github.com/d12frosted/vulpea/discussions/106) to `vulpea-note`. I literally was [disappointed](https://github.com/d12frosted/vulpea/discussions/106#discussioncomment-1601429) with the performance, and I decided to take the path of least resistance and provided some specialised queries (because they kind of solve the problem for my most-used patterns).
...
What exactly makes specialised queries so fast? We start by narrowing down all notes in the fastest possible way. Then, we exchange this list of IDs for a list of notes in a single transaction.
...
Ignore the serialisation of the string data. It is related to [emacsql limitations](https://github.com/skeeto/emacsql#limitations).

Now that we have the list of IDs, we can get a list of notes in one transaction by using `vulpea-db-query-by-ids`.

## `*-every`

We use the [INTERSECT operator](https://www.sqlite.org/lang_select.html#compound_select_statements) to get notes tagged by all provided tags. Looks trickier than the `IN` operator, but the idea is simple. We find a list of notes tagged by the first tag, a list of notes tagged by the second tag, a list of notes tagged by the third tag, and so on… And then we calculate the intersection of these lists. Voilà!
...
- Improve `filetags` handling:
  - Properties format them with `:` as separator;
  - Remove the property when setting them to an empty list instead of leaving an empty property.
- Allow configuring the candidates source for the `vulpea-find` function via the `vulpea-find-default-candidates-source` variable.
- New function to select from an arbitrary list of notes as opposed to relying on a filter - `vulpea-select-from`.
...
Vulpea already offers a lot of features. Whilst working on other projects that use Vulpea, I discover new missing features that I would love to implement. And the issue tracker is not empty. Unless something changes, my plan is the following:
